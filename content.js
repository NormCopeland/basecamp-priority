...
function addPriorityButtons() {
    ...
    todos.forEach(todo => {
        ...
        chrome.storage.sync.get(['accountId'], (result) => {
            if (!result.accountId) {
                console.log('No account ID set, skipping priority button');
                return;
            }

            // Get the recording ID from the task element
            const recordingId = todo.getAttribute('data-recording-id');
            console.log('Recording ID:', recordingId);
            if (!recordingId) {
                console.log('No recording ID found, returning');
                return;
            ...
            form.className = 'button_to priority-form';
            form.method = 'post';
            form.action = `/${result.accountId}/my/priorities?group_by=bucket`;
            ...
            if (indentDiv) {
                ...
                indentDiv.insertBefore(container, indentDiv.firstChild);
